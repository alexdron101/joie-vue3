...
onMounted(() => {
    ...
    /* Начало импорт АПИ*/
    fetch('https://new.joie.com.ua/api/get-page-site')
        .then(response => response.json())
        .then(data => {
            let formatedData = {};
            data.forEach(it => {
...
<template>
    ...
    <div>
        ...
        <div class="home-banner small" id="top-block">
            ...
            <h2>
                <template v-if="lang === 'ua'">РОЗРОБКА САЙТУ <br>ДЛЯ БІЗНЕСУ</template>
                <template v-if="lang === 'ru'">РАЗРАБОТКА САЙТА<br>ДЛЯ БИЗНЕСА</template>
                <template v-if="lang === 'en'">WEBSITE DEVELOPMENT<br>FOR BUSINESS</template>
            </h2>
        ...
        <span v-html="pages.id7 && pages.id7['text_' + lang]"></span>
        ...
        <section id="sect6" class="">
            ...
            <div id="prices" class="prices-home">
                <span v-html="pages.id8 && pages.id8['text_' + lang]"></span>
                <template v-if="lang === 'ua'">
        ...
        <span v-html="pages.id9 && pages.id9['text_' + lang]"></span>
